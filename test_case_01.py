...
dataset_dic = './dataset/'
pre_dataset = './dataset_test_case_01'
IoT_list = ['Temperature_and_Humidity', 'Water_Level']
features = ['src.port', 'flow_duration', 'mqtt_duration', 'mqtt_connection_duration', 'mqtt_connection_ack_duration', 'mqtt_disconnection_duration', 'IoT_label']
            ...
            if row['tcp.connection.fin'] == 1 and row['mqtt.msgtype'] != 14:
                ...
                if int(int(row['tcp.dstport'])) in session_dict:
                    if len(session_dict[int(row['tcp.dstport'])]) == 4:
                        ...
                        new_list.append([int(row['tcp.dstport']), flow_duration, mqtt_duration, mqtt_connection_duration, mqtt_connection_ack_duration, mqtt_disconnection_duration, IoT])
...
# <-- train set split -->
train_features = features[1:-1] #Excepting src.port & label
